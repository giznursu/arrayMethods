...
// CODE HERE
const evenNumbers = mixedNumbers.filter((element,index) => element%2 === 0)
...
// CODE HERE
const postTaxPrices = prices.map((element,index,arr)=>element*1.07)
...
// CODE HERE
const myStrongest = monstersInYourPocket.filter((element,index,arr)=>{
...
const totals = orders.filter((element,index,arr)=>{
  (element.price*(element.tax+1))
})
//console.log(totals)
////////// PROBLEM 6 //////////
...
const bobsTotal = purchases.filter((element,index,arr)=> element.owner = "Bob").reduce((acc,curr)=>acc+curr.price,0)
